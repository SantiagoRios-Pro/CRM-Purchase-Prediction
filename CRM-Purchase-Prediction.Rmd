...
We read in the dataset and examine its structure to understand the variables and data types.
...
We separate customer sessions that resulted in revenue from those that did not.
...
We create a new binary column to simplify classification: success (1) or not (0).
...
We normalize continuous variables so they are on the same scale, which improves model performance.
...
We split the dataset for training and testing to evaluate model performance.
...
We train a decision tree to predict purchase outcomes and visualize the model.
...
We build a more robust model using Random Forest, evaluate its performance, and analyze which features are most important for predicting purchases.
